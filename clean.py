...
# Automatically align working directory to script location
...
def parse_date(d_str):
    ...
    # Parse 6-digit integer string (YYYYMM)
    if re.match(r'^\d{6}$', d_str):
        y, m = int(d_str[:4]), int(d_str[4:])
        if 1 <= m <= 12:
            return pd.Timestamp(year=y, month=m, day=1)

    # Parse 2-digit year format (Mon-YY)
    ...
    # Parse standard slash format (MM/DD/YY)
    ...
    # Fallback ISO timestamp parsing
...
def run_pipeline(input_filepath="global_temp_dirty_v2.csv", output_filepath="cleaned_monthly.csv"):
    """
    Main ETL Pipeline execution wrapper for external API integration.
    """
    ...
    for idx, line in enumerate(lines[1:], start=2):
        ...
        # Handle transposed column anomalies
        if dt is None:
    ...
    # Sort time series and deduplicate records
    df_sorted = df.sort_values('date')
    df_dedup = df_sorted.drop_duplicates(subset=['date'], keep='first').copy()

    # Apply Interquartile Range (IQR) for outlier detection
    ...
    # Reindex onto contiguous monthly grid and apply linear interpolation
    ...
    # Baseline normalization and standardized Z-score transformation
    ...
    # Export clean metrics dataset
    df_export = df_full.reset_index()
    df_export['date'] = df_export['date'].dt.strftime('%Y-%m')
    df_export[['date', 'anomaly_c', 'z']].to_csv(output_filepath, index=False)
    ...
    # Render dual-encoded visualization figure
